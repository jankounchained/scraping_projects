...
# scraping loop
```{r}
scraper <- function(link, timeout = sample(5, 1) + rnorm(1)){
...
# cleaning the scrape
```{r}
var_extract <- function(vector) {
  
  df_clean <- tibble(var = as.character(vector))  %>%
    separate(var, 
    ...
    mutate(etage = as.character(str_extract_all(rest, "etage: ......")),
           etage = tolower(etage),
           etage = case_when(str_detect(etage, "stuen|st") ~ "ground",
           ...
           rent = str_extract_all(rent, "[0-9]"),
           rent = paste(unlist(rent), collapse = ''),
           # indskud
           indskud = str_extract(rest, "Indskud:.........."),
           indskud = str_remove_all(indskud, "\\."),
           indskud = str_extract_all(indskud, "[0-9]"),
           indskud = paste(unlist(indskud), collapse = ''),
           # deposit
           deposit = str_extract(rest, "Depositum:.........."),
           deposit = str_remove_all(deposit, "\\."),
           deposit = str_extract_all(deposit, "[0-9]"),
           deposit = paste(unlist(deposit), collapse = ''),
           # advance_rent
           advance_rent = str_extract(rest, "Forudbetalt leje:.........."),
           advance_rent = str_remove_all(advance_rent, "\\."),
           advance_rent = str_extract_all(advance_rent, "[0-9]"),
           advance_rent = paste(unlist(advance_rent), collapse = ''),
           # ac_usage
           ac_usage = str_extract(rest, "A/C-forbrug:....."),
           ac_usage = str_remove_all(ac_usage, "\\."),
           ac_usage = str_extract_all(ac_usage, "[0-9]"),
           ac_usage = paste(unlist(ac_usage), collapse = ''),
           # avaiable 18 char
